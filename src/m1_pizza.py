...
def run_test_generate_points_on_circle():
    ...
    # ------------------------------------------------------------------
    # My Test 1:
    expected = [rg.Point(77.0, 40.0),  # All numbers are approximate.
                rg.Point(27.0, 90.0),
                rg.Point(-23.0, 40.0),
                rg.Point(27.0, -10.0)]
    circle = rg.Circle(rg.Point(27, 40), 50)
...
def run_test_draw_points_on_circle():
    ...
    # ------------------------------------------------------------------
    # My Test 1:
    title = 'DRAW_POINTS_ON_CIRCLE, my test 1:  75 purple dots.'
    window = rg.RoseWindow(500, 500, title)
    circle = rg.Circle(rg.Point(175, 200), 125)
...
def draw_points_on_circle(window, circle, number_of_points, color):
    ...
    for k in range(len(centers)):
        circle_set = rg.Circle(centers[k], 10)
        circle_set.fill_color = color

        circle_set.attach_to(window)
...
def run_test_pizza():
    ...
    # ------------------------------------------------------------------
    # My Test 1:
    title = 'PIZZA my test 1:  5 slices, 200, thin black lines on a yellow circle.'
    window = rg.RoseWindow(600, 600, title)
    circle = rg.Circle(rg.Point(225, 325), 200)
...
def run_test_polygon():
    ...
    # ------------------------------------------------------------------
    # My Test 1 (on new window):
    title = 'My POLYGON tests 1:  4 segments, black lines on lime green circle .'
    window = rg.RoseWindow(550, 400, title)
...
def run_test_fancy_polygon():
    ...
    # ------------------------------------------------------------------
    # My Test 1 (on another window):
    title = 'My FANCY POLYGON test 1:  300 lines, hops = 61.'
    window = rg.RoseWindow(480, 350, title)

    circle = rg.Circle(rg.Point(240, 165), 150)
    circle.fill_color = 'firebrick'
    fancy_polygon(window, circle, 300, 61, 'blue', 5)
    window.close_on_mouse_click()
